...
class GodtierExecutor:
    ...
    def __init__(self, topic: str, mode: str = "full", skill_dir: str = None,
        ...
        if skill_dir:
            ...
            # CRIT-01：__file__ = .../scripts/orchestration/executor.py，需上溯 3 级到技能根
            # 旧实现仅上溯 2 级到 scripts/，导致 detectors/workflows 等路径全部错位
            self.skill_dir = os.path.dirname(os.path.dirname(os.path.dirname(__file__)))
    ...
    def phase1_collect(self) -> list[dict]:
        ...
        主Agent执行步骤：
        1. 调用此方法获取Agent配置列表
        2. 对每个Agent配置，执行 delegate_task(task=agent["prompt"])
        3. 所有Agent同时启动（不要串行！）

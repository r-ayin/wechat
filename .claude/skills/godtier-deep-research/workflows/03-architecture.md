...
**执行模式**：主Agent自身就是编排器。不是运行一个脚本全自动执行，而是主Agent读取executor.py的指引，分步调用sessions_spawn，管理状态，处理异常。
...
**阶段内并行**：
- 阶段1的所有采集Agent必须同时启动（一次性调用所有sessions_spawn）
- 阶段2的所有分析Agent必须同时启动
...
|------|---------|---------|
| 规划 | read, write | - |
| 采集 | web_search, web_fetch, sessions_spawn | browser, pdf |
| 分析 | sessions_spawn, exec, read/write | browser |

...
0. **统一编排入口** — 所有 agent 通过 `scripts/pipeline.py` + `wechat-pipeline` Skill 驱动管线（agent-loop：`init`→`next`→派发子 agent/跑门禁→`mark`→循环）。**禁止**主上下文手动推进阶段、手动跑门禁、手动写阶段产出；主上下文只持 step 元数据，不接触内容。从任意点恢复用 `init --from <phase>`。详见 `.claude/skills/wechat-pipeline/SKILL.md`。
...
> 圆圈编号 → Phase 映射：①=Phase 0 竞品/选题，②=Phase 2 godtier 研究（Phase 1 研究简报为其前置），③=Phase 3 persona 重写，③.5=QA 门禁，④=Phase 4 输出。
